            ...
            <motion.div variants={fadeUp}>
              ...
              <div className="space-y-1">
                {project.roles.map((role) => (
                  <p key={role} className="text-muted">{role}</p>
                ))}
              </div>
            </motion.div>
                ...
                {project.gallery.map((image, index) => (
                  <button
                    ...
                    className={`aspect-video rounded-lg overflow-hidden border-2 transition-colors ${
                      selectedImage === index ? 'border-accent' : 'border-surface'
                    }`}
                  >
                    <img
                      src={image}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="text-muted leading-relaxed whitespace-pre-line">
              {project.content || 'Detailed case study content will be available soon.'}
            </div>
          </motion.div>
        </div>
      </section>
